package vn.com.processman.modules.processexec.base.model;
...
import domainapp.basics.model.config.module.containment.ScopeDef;
...
/**
 * @overview
 *  Represents an execution of a {@link Process} or specific components thereof. 
 *  
 * @author dmle
 */
@DClass(serialisable=false)
public class ProcessExecution implements ProcessUserActivity {
  
  /**extended scope definitions */
  public static final ScopeDef ScopeDefProcess = new ScopeDef(
      Process.class,
      new String[] {Process.A_codeDef, Process.A_nameDef, Process.A_tasks},Boolean.FALSE); // end ScopeDefProcess

  public static final ScopeDef ScopeDefTask = new ScopeDef(
      Task.class,
      new String[] {Task.A_self, Task.A_codeDef, Task.A_nameDef, Task.A_process, Task.A_status},Boolean.FALSE); // end ScopeDefTask
  ...
  public static final String A_processName = "processName";
  public static final String A_processes = "processes";
  public static final String Assoc_ProcessExecutionAndProcess = "ProcessExecution-shows-Process";
...
//      associate = @AssocEnd(type = File.class, cardMin = 1, cardMax = 1))  
  ...
  @DAttr(name=A_processName, type=Type.String, length=255, optional=false)
  ...
  @DAssoc(ascName = Assoc_ProcessExecutionAndProcess, role = "processExecution", 
      ascType = AssocType.One2Many, endType = AssocEndType.One, 
      associate = @Associate(type = Process.class, cardMin = 0, cardMax = MetaConstants.CARD_MORE))
  ...
  public ProcessExecution(Integer id, OrgUnit orgUnit, String processName, Semester semester, Integer year) {
    this.id = nextID(id);
  ...
  public ProcessExecution(OrgUnit orgUnit, String processName, Semester semester, Integer year) {
    this(null, orgUnit, processName, semester, year);
  }
...
/*  @Metadata(type=Metadata.Type.MethodValueAdderNew)
  ...
  */
  ...
  public String toString() {
    return this.getClass().getSimpleName()+" (" + id + ", " + orgUnit + ", " + processName
        + ", " + semester + ", " + year + ")";
  }
